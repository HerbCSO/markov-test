...
model = SentenceGenerator::MarkovModel.new()
ARGV.each do |file|
  puts "Training on file #{file}"
  model.train(File.read(file))
end
5.times { puts model.complete_sentence }
model.dump_model
